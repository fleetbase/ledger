...
class WalletService
{
    ...
    public function withdraw(
        ...
        if (!$wallet->hasSufficientBalance($amount)) {
            throw new \Exception(
                "Insufficient balance in wallet [{$wallet->public_id}]. " .
                "Available: {$wallet->balance}, Required: {$amount}."
            );
        }
    ...
    public function transfer(
        ...
        if (!$fromWallet->hasSufficientBalance($amount)) {
            throw new \Exception(
                "Insufficient balance in source wallet [{$fromWallet->public_id}]. " .
                "Available: {$fromWallet->balance}, Required: {$amount}."
            );
        }
